...
def checkin():
    ...
    db.session.commit()
    flash(f'✅ Checked in! Attendance streak: {streak.attendance_streak if streak else 1} 🔥', 'success')
    return redirect(url_for('attendance.index'))
...
def checkout():
    ...
    if att:
        ...
        db.session.commit()
        flash(f'👋 Checked out! Session: {att.duration_mins or "—"} mins', 'success')
    else:
