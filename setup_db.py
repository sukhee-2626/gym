...
def seed_database(app_instance=None):
    ...
    with app.app_context():
        # Do not drop_all in a shared script, only create
        db.create_all()

        # Seed membership plans
        from app.models import MembershipPlan, Exercise, User, Streak
        import uuid
        ...
        # Seed demo user
        if User.query.filter_by(email='demo@smartgym.ai').first() is None:
            demo = User(
        ...
        # Seed admin user
        if User.query.filter_by(email='admin@smartgym.ai').first() is None:
            admin = User(
